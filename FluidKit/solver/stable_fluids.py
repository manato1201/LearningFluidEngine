...
class StableFluids2D:
    ...
    def _project(
        ...
        試みたが不採用とした。境界条件の導出自体は正しく検証済み
        (Neumann-copy 境界を畳み込んだ5点ステンシル行列の解は
        3000回 Gauss-Seidel の参照解と 1e-12 オーダーで一致、
        発散量も旧実装と同等かそれ以上)。しかし純 Neumann 境界のため
        行列が特異 (ヌル空間=定数ベクトル) で、CG に warm-start を
        使うと丸め誤差がヌル空間方向に蓄積し数十ステップで発散した。
        warm-start なし・RHSのヌル空間成分除去 (`b -= b.mean()`) で
        数値的には安定させられたが、疎行列 CG は今回のグリッド規模
        (N=64/128) では GS より大幅に遅く
        (N=64: 旧 2.6ms → 新 14.8ms、N=128: 旧 7.9ms → 新 508ms、
        capped-iteration 版でも N=128 で旧の約4倍遅い) 、
        計画の目標 (2〜5倍高速化) を満たせなかったため差し戻した。
        """
        N = self.N
        p   = np.zeros_like(u)
        div = np.zeros_like(u)
