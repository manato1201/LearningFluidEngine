...
│   │   ├── collect_data.py     学習データ生成 (cKDTree で近傍を事前計算)
│   │   ├── train_v2.py         訓練 (AMP + CosineAnnealing, mmap データセット)
...
│   ├── tests/              ★ pytest テストスイート (NEW)
│   │   ├── test_solvers.py     運動量保存・境界貫通なし等の物性テスト
│   │   ├── test_reproducibility.py  再現性ハッシュ一致テスト
│   │   └── test_io.py          JSON/NPZ ラウンドトリップテスト
...
│       ├── algorithms.html
│       ├── ALGORITHMS.md
│       ├── PERF_RESULTS.md     ★ 性能改善の実測記録 (NEW)
│       └── lecture.html        完全講義資料
└── .github/
    └── workflows/
        ├── release.yml     Emscripten ビルド + GitHub Release 自動配布
        └── test.yml        ★ push/PR ごとの pytest 自動実行 (NEW)
```
...
| [docs/PERF_RESULTS.md](FluidKit/docs/PERF_RESULTS.md) | 性能改善の実測記録（solver 再利用化・KNN 事前計算・I/O 圧縮等） |
